...
function ClassCard() {
   ...
   const toClassDetail = (code) => {history.push(`classlist/details/${code}`)};
            ...
            if (classList.archived == 'Active') {
               return (
                  <Card className="classCard" key={classList.class_code}>
                     <EditIcon id="editIconClassCard" onClick={() => handleClick(classList)}/>
                     <h2 id="classCardTitle" onClick={() => toClassDetail(classList.class_code)}>
                        {classList.name}
                     </h2>
