...
router.get("/homefeed", (req, res) => {
  ...
  pool
    ...
    .catch((err) => {
      console.log("error getting user's class stemtells", err);
      res.sendStatus(500);
    });
});
/**
  ...
  (async () => {
    ...
    try {
      ...
      const queryTextAddStemtell = `
          ...
          RETURNING id
          `;
      const response = await client.query(queryTextAddStemtell, [
        newStemtell.class_id,
        user.id,
        newStemtell.title,
        newStemtell.body_text,
        newStemtell.media_url,
      ]);
