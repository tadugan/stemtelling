...
router.get('/', rejectUnauthenticated, (req, res) => {
  ...
  const query= `SELECT "user".name AS username, "stemtell".id, "user".profile_picture_url, "comment".comment, "comment".date_published, "comment".id, "comment".stemtell_id, "user".authority 
  FROM "comment"
  JOIN "user" ON "comment".user_id = "user".id
  JOIN "stemtell" ON "stemtell".id = "comment".stemtell_id
  WHERE  "user".authority = 'student' 
  AND "comment".stemtell_id = 2;`;
  pool
